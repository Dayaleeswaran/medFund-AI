import { justPayInitiateTransaction } from "@/lib/fintech/justpay";
import {
  fintechJson,
  handleFintechError,
...
export async function POST(req: Request) {
  ...
  try {
    const result = await justPayInitiateTransaction(body);
    if (!result.ok) {
      return fintechJson(
        { error: "JustPay payment failed", status: result.status, data: result.data },
        502,
      );
    }
