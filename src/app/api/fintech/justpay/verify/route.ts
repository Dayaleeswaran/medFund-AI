import { justPayVerifyRegistration } from "@/lib/fintech/justpay";
import {
  fintechJson,
  handleFintechError,
...
export async function POST(req: Request) {
  ...
  try {
    const result = await justPayVerifyRegistration(body);
    if (!result.ok) {
      return fintechJson(
        { error: "JustPay verify failed", status: result.status, data: result.data },
        502,
      );
    }
